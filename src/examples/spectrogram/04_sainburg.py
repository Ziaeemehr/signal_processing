...
def stft(X, fftsize=128, step=65,
         mean_normalize=True, real=False,
...
def pretty_spectrogram(d, log=True, thresh=5, fft_size=512, step_size=64):
    ...
    """
    specgram = np.abs(
        stft(d, fftsize=fft_size, step=step_size,
...
def invert_pretty_spectrogram(X_s, log=True, fft_size=512,
                              step_size=512 / 4, n_iter=10
                              ):
...
def create_mel_filter(fft_size, n_freq_components=64,
                      start_freq=300,
                      end_freq=8000,
                      samplerate=44100
                      ):
